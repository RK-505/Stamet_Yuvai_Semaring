...
# ===== Fungsi: Ambil waktu GFS terbaru =====
...
st.sidebar.info(f"📅 GFS otomatis: {run_date} {run_hour}Z")
...
if "pratesfc" in parameter:
    var = ds["pratesfc"][forecast_hour] * 3600  # konversi ke mm/jam
    label = "Curah Hujan (mm/jam)"
    cmap = "Blues"
elif "tmp2m" in parameter:
    var = ds["tmp2m"][forecast_hour] - 273.15  # Kelvin ke Celcius
    label = "Suhu (°C)"
    cmap = "coolwarm"
elif "ugrd10m" in parameter:
    u = ds["ugrd10m"][forecast_hour]
    v = ds["vgrd10m"][forecast_hour]
    var = ((u ** 2 + v ** 2) ** 0.5) * 1.94384  # m/s ke knot
    ...
    st.warning("Parameter tidak tersedia.")
    st.stop()

# ===== Fokus ke Wilayah Krayan =====
...
# --- Plot data (paling aman via xarray) ---
if is_contour:
